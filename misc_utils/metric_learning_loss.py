...
def loss_select(loss, opt, to_optim):
    ...
    if loss == "triplet":
        ...
        loss_params = {
            "num_proxies": opt.nclass,
            "embedding_dim": opt.classembed
            if "num_cluster" in vars(opt).keys()
            else opt.embed_dim,
        }
        criterion = ProxyNCALoss(**loss_params)
        to_optim += [{"params": criterion.parameters(), "lr": opt.proxy_lr}]
    elif loss == "crossentropy":
        loss_params = {"n_classes": opt.nclass, "inp_dim": opt.embed_dim}
        criterion = CEClassLoss(**loss_params)
        to_optim += [
            {"params": criterion.parameters(), "lr": opt.lr, "weight_decay": 0}
        ]
    else:
...
class Sampler:
    ...
    def randomsampling(self, batch, labels):
        ...
        sampled_triplets = [
            [
                x
                for x in list(it.product(*[class_dict[j] for j in i]))
                if x[0] != x[1]
            ]
    ...
    def semihardsampling(self, batch, labels):
        ...
        sampled_triplets = [
            [a, p, n] for a, p, n in zip(anchors, positives, negatives)
        ]
        return sampled_triplets

    def distanceweightedsampling(
        self, batch, labels, lower_cutoff=0.5, upper_cutoff=1.4
    ):
        """
        This methods finds all available triplets in a batch given by the classes provided in labels, and select
        ...
        sampled_triplets = [
            [a, p, n] for a, p, n in zip(list(range(bs)), positives, negatives)
        ]
        return sampled_triplets
    ...
    def npairsampling(self, batch, labels):
        ...
        sampled_npairs = [
            [a, p, *list(neg)] for (a, p), neg in zip(pos_pairs, neg_tuples)
        ]
    ...
    def inverse_sphere_distances(self, batch, dist, labels, anchor_label):
        ...
        log_q_d_inv = (2.0 - float(dim)) * torch.log(dist) - (
            float(dim - 3) / 2
...
class NPairLoss(torch.nn.Module):
    ...
    def npair_distance(self, anchor, positive, negatives):
        ...
        return torch.log(
            1
            + torch.sum(
                torch.exp(anchor.mm((negatives - positive).transpose(0, 1)))
            )
        )
    ...
    def forward(self, batch, labels):
        ...
        loss = loss + self.l2 * torch.mean(
            torch.stack(
                [
                    self.weightsum(batch[npair[0], :], batch[npair[1], :])
                    for npair in sampled_npairs
                ]
            )
        )
...
class MarginLoss(torch.nn.Module):
    ...
    def forward(self, batch, labels):
        ...
        for triplet in sampled_triplets:
            ...
            pos_dist = (
                (train_triplet["Anchor"] - train_triplet["Positive"])
                .pow(2)
                .sum()
                + 1e-8
            ).pow(1 / 2)
            neg_dist = (
                (train_triplet["Anchor"] - train_triplet["Negative"])
                .pow(2)
                .sum()
                + 1e-8
            ).pow(1 / 2)
        ...
        pair_count = torch.sum((pos_loss > 0.0) + (neg_loss > 0.0)).type(
            torch.cuda.FloatTensor
        )

        # Actual Margin Loss
        loss = (
            torch.sum(pos_loss + neg_loss)
            if pair_count == 0.0
            else torch.sum(pos_loss + neg_loss) / pair_count
        )
...
class ProxyNCALoss(torch.nn.Module):
    def __init__(self, num_proxies, embedding_dim):
        ...
        self.PROXIES = torch.nn.Parameter(
            torch.randn(num_proxies, self.embedding_dim) / 8
        )
    ...
    def forward(self, batch, labels):
        ...
        pos_proxies = torch.stack(
            [PROXIES[pos_label : pos_label + 1, :] for pos_label in labels]
        )
        neg_proxies = torch.stack(
            [
                torch.cat(
                    [
                        self.all_classes[:class_label],
                        self.all_classes[class_label + 1 :],
                    ]
                )
        ...
        neg_proxies = torch.stack(
            [PROXIES[neg_labels, :] for neg_labels in neg_proxies]
        )
        # Compute Proxy-distances
        dist_to_neg_proxies = torch.sum(
            (batch[:, None, :] - neg_proxies).pow(2), dim=-1
        )
        dist_to_pos_proxies = torch.sum(
            (batch[:, None, :] - pos_proxies).pow(2), dim=-1
        )
        # Compute final proxy-based NCA loss
        negative_log_proxy_nca_loss = torch.mean(
            dist_to_pos_proxies[:, 0]
...
class CEClassLoss(torch.nn.Module):
    ...
    def forward(self, batch, labels):
        ...
        """
        return self.ce_loss(
            self.mapper(batch), labels.type(torch.cuda.LongTensor)
        )
