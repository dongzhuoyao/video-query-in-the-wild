...
class arv1002080tripletclsrankfsmixv2(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if random.uniform(0, 1)>=0.5:#do mixup, output three normal videos and one mixed videos
            ...
            mix = anchor*_ratio + positive*(1-_ratio)
            triple = np.concatenate([anchor,positive,negative, mix],axis=0)
            meta['labels']=[self.cls2int[anchor_class_name], self.cls2int[anchor_class_name], self.cls2int[negative_class_name],self.cls2int[anchor_class_name]]
        else:#
            anchor, negative = random.sample(self.data_dict[self.split][anchor_class_name], 2)

            anchor = _read(anchor)[np.newaxis, :]
            negative = _read(negative)[np.newaxis, :]
            _ratio = random.uniform(0.6, 0.9)
            positive = anchor * _ratio + negative * (1 - _ratio)
            triple = np.concatenate([anchor, positive, negative, positive], axis=0)
            meta['labels'] = [self.cls2int[anchor_class_name], self.cls2int[anchor_class_name],
                              self.cls2int[anchor_class_name], self.cls2int[anchor_class_name]]
        return triple, meta
