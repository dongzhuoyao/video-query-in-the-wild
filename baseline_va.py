...
def train(loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (input, meta) in tqdm(enumerate(part(loader, iter_size)), desc='Train Epoch'):
        ...
        #output, metric_feat,_, _ = model(input, meta)
        logits, rank_embed, nled_logits, _, reg_logits= model([input,target,temperature],meta)
        #logits, rank_embed, nled_logits, nled_embed, reg_logits
        ce_loss = ce_loss_criterion(nled_logits, target)
        register_loss = ce_loss_criterion(reg_logits, target)
        ...
        register_losses.update(register_loss.item())
        ...
        if i % args.print_freq == 0 and i>0:
            logger.info('[{0}][{1}/{2}({3})]\t'
                        ...
                        'RegisterLoss={register_loss.avg:.4f}\t'
                        'LR={cur_lr:.7f}\t'
                        'bestAP={ap:.3f}'.format(
                epoch, i, int(len(loader) * iter_size), len(loader), timer=timer,
                data_time=data_time, loss=losses, ce_loss=ce_losses,register_loss=register_losses, ap=args.best_score,
                cur_lr=cur_lr, warming=warming))
            losses.reset()
            ce_losses.reset()
            register_losses.reset()
        del loss, nled_logits  # make sure we don't hold on to the graph
...
def val(args, model, triple_eval=False):
    ...
    def feat_func(input):
        _, metric_feat, _, _, reg_logits= model([input,None,temperature],{})  # [B,C]
