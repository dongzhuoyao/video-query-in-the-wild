...
class VRActivityNet(data.Dataset):
    ...
    def load_data(self):
        ...
        if "d300" in self.args.semantic_json:
            self.semantic_mem = np.zeros(
                (self.args.nclass, 300), dtype=np.float32
            )
        elif "d200" in self.args.semantic_json:
            self.semantic_mem = np.zeros(
                (self.args.nclass, 200), dtype=np.float32
            )
        elif "d1024" in self.args.semantic_json:
            self.semantic_mem = np.zeros(
                (self.args.nclass, 1024), dtype=np.float32
            )
        else:
            raise
    ...
    def __getitem__(self, index):
        ...
        negative_class_name = random.choice(
            list(set(self.cur_label_list) - set(list(anchor_class_name)))
        )
        meta = {}
        ...
        meta["labels"] = [
            self.cls2int[anchor_class_name],
            self.cls2int[anchor_class_name],
            self.cls2int[negative_class_name],
        ]
        meta["label_names"] = [
            anchor_class_name,
            anchor_class_name,
...
def _pre_process(cur_video_list, input_size, test_frame_num):  # time-consuming
    ...
    for cur_video in cur_video_list:
        ...
        if frame_num == 0:
            raise  # bad data in activitynet-validation
        images = read_video(
            frame_path=frame_path,
            start_frame_idx=start_frame_idx,
            gt_frame_num=frame_num,
            train_frame_num=test_frame_num,
...
def generate_multi_query(query_list):
    ...
    for i, q in enumerate(query_list):
        ...
        same_type_videos = [
            _ for _ in same_type_videos if _["video_id"] != q["video_id"]
        ]
    ...
    logger.warning(
        "randomness check: {}".format(
            [q["video_id"] for q in new_query_list[0]]
        )
    )
...
class evaluation_metric:
    ...
    def multiprocess_calculate(self, single_query_hit):
        ...
        for _thres in self.r_at_n:
            ...
            cur_recall = (
                sum([i["tp"] for i in single_query_hit[:_thres]])
                * 1.0
                / single_query_npos
            )
    ...
    def multiprocess_update(self, cls_name, retrieval_type, r_dict):
        ...
        for _, _thres in enumerate(self.r_at_n):
            ...
            self.avg2_full_retrieval_top[str(_thres)][cls_name].append(
                recall_list[_]
            )
    ...
    def add2dict(self, cls_name, retrieval_type, single_query_hit):
        ...
        if "gt_label" in single_query_hit[0]:
            self.cm_gt_labels.extend(
                [i["gt_label"] for i in single_query_hit[:100]]
            )
        if "label" in single_query_hit[0]:
            self.cm_labels.extend([i["label"] for i in single_query_hit[:100]])
        if (
            "query_frame_info" in single_query_hit[0]
            and "candidate_frame_info" in single_query_hit[0]
        ):
        ...
        if "query_duration_sec" in single_query_hit[0]:
            self.query_duration_map_dict[
                single_query_hit[0]["query_duration_sec"]
            ] = ap
    ...
    def get_result(self, pass_content=[]):
        for _thres in self.r_at_n:
            self.full_retrieval_top[str(_thres)] = Average(
                self.full_retrieval_top[str(_thres)]
            )
            self.base_retrieval_top[str(_thres)] = Average(
                self.base_retrieval_top[str(_thres)]
            )
            ...
            for cls_name, ap_list in self.avg2_full_retrieval_top[
                str(_thres)
            ].items():
                self.avg2_full_retrieval_top[str(_thres)][cls_name] = Average(
                    ap_list
                )  # list to value
        ...
        o2_class_specific_base_map = Average(
            [self.class_dict[_cls_name] for _cls_name in self.base_classes]
        )
...
class ARV_Retrieval_Clip:
    ...
    def load_data(self):
        data_dict = json.load(
            open(dataset_config[self.args.meta_split]["moment_eval_json_path"])
        )
        self.query_list = []
        ...
        logger.warning(
            "query length={}, gallery size={}".format(
                len(self.query_list), len(self.gallery_list)
            )
        )
    ...
    def extract_item_feature(self):
        ...
        cache_path = os.path.join(
            logger.get_logger_dir(), "clip_feat_cache.pkl"
        )
        if os.path.exists(cache_path) and self.args.read_cache_feat:
            ...
            for idxx, data_batch in tqdm(
                ...
                if self.args.debug and idxx > debug_iter:
                    break

                if self.args.memory_leak_debug:
                    feat = np.random.rand(
                        len(data_batch), self.feat_dim, self.test_frame_num
                    ).astype(np.float32)
                else:
                    img = _pre_process(
                        data_batch, self.input_size, self.test_frame_num
                    )
            ...
            def garner_feat(_g, clip_sec=self.clip_sec):
                ...
                def cal_label(loc_sec):
                    ...
                    for ann in annotations:
                        ...
                        if (
                            seg[0] <= loc_sec[0]
                            and seg[1] >= loc_sec[1]
                            and label in self.possible_classes
                        ):  # if inside
                            target_label = label
                            break
                    return target_label
                ...
                for clip_start in range(
                    0, int(_g["activitynet_duration"]), clip_sec
                ):
                    loc_feat = [
                        clip_start * fps // self.temporal_stride,
                    ...
                    r_list.append(
                        dict(
                            ...
                            feat=np.mean(
                                _g["feat"][:, loc_feat[0] : loc_feat[1]], axis=1
                            ),
            ...
            logger.warning(
                "average #segment={} per candidate video.".format(
                    len(self.gallery_list) / proceeded_id
                )
            )
    ...
    def ranking(self,):
        logger.warning(
            "start ranking, query size={}, gallery clips size={}".format(
                len(self.query_list), len(self.gallery_list)
            )
        )
        ...
        for _ in tqdm(
            range(len(self.query_list)),
            total=len(self.query_list),
            desc="eval_clips, ranking",
        ):
            ...
            for idx, candidate in enumerate(self.gallery_list):
                ...
                if (
                    candidate["clip_label"] == gt_label
                ):  # mainly evaluate R@N at tIoU=0.5
                    scored_dict["tp"] = 1
                else:
                    scored_dict["tp"] = 0
            ...
            D, I = self.faiss_index.search(
                query_feat.reshape(1, query_feat.shape[0]),
                k=self.faiss_xb.shape[0],
            )
            D = np.squeeze(D)
            I = np.squeeze(I)
            ...
            self.class_map_evaluation.add2dict(
                cls_name=gt_label,
...
class ARV_Retrieval_Moment:
    ...
    def extract_item_feature(self):
        ...
        cache_path = os.path.join(
            logger.get_logger_dir(), "moment_feat_cache.pkl"
        )
        if os.path.exists(cache_path) and self.args.read_cache_feat:
            file = open(cache_path, "rb")
            object_file = pickle.load(file)
            ...
            def garner_feat(_g, clip_length_sec=5, max_clip_per_moment=26):
                ...
                for clips_per_moment in range(1, max_clip_per_moment + 1):
                    for moment_start_sec in range(
                        0,
                        int(_g["activitynet_duration"])
                        - clip_length_sec * clips_per_moment,
                        clip_length_sec,
                    ):
                        moment_duration_sec = clip_length_sec * clips_per_moment

                        loc_feat = [
                            moment_start_sec * fps // self.temporal_stride,
            ...
            logger.warning(
                "#moment/video = {}".format(
                    sum(moment_total) / len(moment_total)
                )
            )
            logger.warning(
                "average #segment={} per candidate video.".format(
    ...
    def ranking(self,):
        logger.warning(
            "start ranking, query size={}, gallery potential moments size={}".format(
                len(self.query_list), len(self.gallery_list)
            )
        )
        ...
        def work(
            id, jobs, result
        ):  # https://gist.github.com/brantfaircloth/1255715/5ce00c58ae8775c7d75a7bc08ab75d5c7f665bca
            while True:
                queries = jobs.get()
                if queries is None:
                    break
                query = queries[0]
                ignore_videoid_list = [
                    q["video_id"] for q in queries
                ]  # ignore those long videos, which current short query video comes from..
                assert query["retrieval_type"] != RETRIEVAL_TYPE_NOISE
                gt_label = query["label"]
                single_query_hit = list()
                for idx, candidate in enumerate(self.gallery_list):
        ...
        logger.warning(
            "multi processing evaluation: #Process={}".format(
                NUMBER_OF_PROCESSES
            )
        )
        ...
        for _ in tqdm(
            range(len(self.query_list)),
            desc="multi-process ranking,#cpu={}".format(NUMBER_OF_PROCESSES),
            total=len(self.query_list),
        ):  # fetch data for len(self.query_list) times
            r = result.get()
            self.class_map_evaluation05.multiprocess_update(
                cls_name=r["cls_name"],
...
class ARV_Retrieval:
    def __init__(self, args, feat_extract_func):
        ...
        self.eval_split = (
            args.eval_split
        )  # no eval_split for clip, moment retrieval, because they are and only are used for testing.
        self.test_batch_size = args.test_batch_size
        self.input_size = args.input_size
        self.test_frame_num = args.test_frame_num
        self.feat_dim = args.metric_feat_dim
        ...
        logger.info(
            "loading {} data: {}".format(
                self.eval_split, len(self.data_list[self.eval_split])
            )
        )
        logger.warning("memory_leak_debug={}".format(args.memory_leak_debug))
        logger.warning("query_num: {}".format(self.args.query_num))
    ...
    def load_data(self):
        if (
            self.eval_split == "training"
        ):  # evaluate on training data for checking
            raise
            self.data_dict = json.load(
                open(dataset_config[self.args.meta_split]["json_path"])
            )
            ...
            for k, v in self.data_dict[self.split].items():
                if k == NOISE_LABEL:
                    self.data_list[self.split].extend(
                        v[:600]
                    )  # experience value
                else:
                    self.data_list[self.split].extend(
                        v[:10]
                    )  # for novel class, the real video number is less than 10.
            return
    ...
    def extract_item_feature(self):
        ...
        if os.path.exists(cache_path) and self.args.read_cache_feat:
            ...
            for idxx, data_batch in tqdm(
                enumerate(chunk_list),
                total=len(chunk_list),
                desc="{}: extracting feat, batch size:{}x{}".format(
                    self.eval_split, self.test_batch_size, len(chunk_list)
                ),
            ):
                if self.args.debug and idxx > 5:
                    break
                if self.args.memory_leak_debug:
                    feat = np.random.rand(
                        len(data_batch), self.feat_dim, self.test_frame_num
    ...
    def ranking(self,):
        logger.warning(
            "start ranking, query size={}, gallery size={}".format(
                len(self.query_list), len(self.gallery_list)
            )
        )
        ...
        for _, queries in tqdm(
            enumerate(self.query_list),
            total=len(self.query_list),
            ...
            for idx, candidate in enumerate(self.gallery_list):
                ...
                scored_dict["gt_label"] = gt_label
                scored_dict["query_duration_sec"] = (
                    query["segment"][1] - query["segment"][0]
                )
                ...
                if (
                    "subclass_eval" in self.args
                    and self.args.subclass_eval == "ggfather"
                ...
                if (
                    "subclass_eval" in self.args
                    and self.args.subclass_eval == "father"
                ):  # evaluation based on class
                    from hierachy_class_util import (
                        is_same_father,
                        is_same_grandgrandfather,
