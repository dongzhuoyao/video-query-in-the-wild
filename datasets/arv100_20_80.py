...
class ARV(data.Dataset):
    def __init__(self, args,transform, input_size,target_transform=None):
        ...
        self.no_novel = False if self.args.no_novel == None else self.args.no_novel
    ...
    def sanity_check(self):
        ...
        print("sanity check, removing {} items, keep {} items".format(len(list(_removed_dict.keys())),len(_new_list)))
    ...
    def load_data(self):
        ...
        for k,v in self.data_dict[self.split].items():
            if k in arv_val_label or k in arv_test_label:#only keep minimal novel class
        ...
        assert len(list(self.cls2int.keys())) == self.args.nclass,"{} not equal {}".format(len(list(self.cls2int.keys())), self.args.nclass)
