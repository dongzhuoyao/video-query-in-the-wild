...
arch = 'resnet18_3d_f2f_memnlfixv3_reg_temp_word4l';temperature=0.1
...
def train(loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (input, meta) in tqdm(enumerate(part(loader, iter_size)), desc='Train Epoch'):
        ...
        output, metric_feat, nled_logits, _, reg_logits, word_logits = model([input,target,temperature,semantic_mem], meta)

        ce_loss = ce_loss_criterion(nled_logits, target)
...
def val(args, model, triple_eval=False):
    ...
    def feat_func(input):
        output, metric_feat, nled_logits, _, reg_logits= model([input,None,temperature, None], {})  # [B,C]
