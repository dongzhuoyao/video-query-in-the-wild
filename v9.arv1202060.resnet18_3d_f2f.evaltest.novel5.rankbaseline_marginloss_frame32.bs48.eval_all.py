...
from retrievel_evaluation import ARV_Retrieval
...
def parse():
    ...
    parser.add_argument('--metric_loss', default=metric_loss, type=str)

    parser.add_argument('--sampling', type=str, default=sampling)
...
def train(loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (input, meta) in tqdm(enumerate(part(loader, iter_size)), desc='Train Epoch'):
        ...
        input = input.view(_batch_size*2, input.shape[2], input.shape[3], input.shape[4], input.shape[5])
        ...
        metric_feat = metric_feat.view(_batch_size*2, metric_feat.shape[1])
        metric_feat = torch.nn.functional.normalize(metric_feat, dim=-1)
        triple_loss = criterion(metric_feat, target)
        loss =  triple_loss
        ...
        if i % args.print_freq == 0 and i>0:
            logger.info('[{0}][{1}/{2}({3})]\t'
                        ...
                        'TripleLoss={triple_loss.avg:.4f}\t'
                        'LR={cur_lr:.7f}\t'
                        'bestAP={ap:.3f}'.format(
                epoch, i, int(len(loader) * iter_size), len(loader), timer=timer,
                data_time=data_time, loss=losses,  triple_loss=triple_losses, ap=args.best_score,
                cur_lr=cur_lr))
            losses.reset()
            #ce_losses.reset()
            triple_losses.reset()
...
def main():
    ...
    if args.evaluate:
        ...
        if args.test_load is not None:
            ...
            weight_path = os.path.join("train_log", "v9.arv1202060.resnet18_3d_f2f.evaltest.novel5.rankbaseline_marginloss_frame32.bs48", "best.pth.tar")
    ...
    if args.optimizer == 'sgd':
        ...
        to_optim = [{'params': model.parameters(), 'lr': args.lr, 'weight_decay': args.weight_decay}]
        from metric_learning_loss import loss_select
        criterion, to_optim = loss_select(loss=args.metric_loss, opt=args, to_optim=to_optim)
