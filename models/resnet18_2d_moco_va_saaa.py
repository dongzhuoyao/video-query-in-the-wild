...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    if pretrained:
        ...
        #TODO, customized load
        new_params = model.state_dict().copy()
        logger.info("model weight keys: {}".format(new_params.keys()))
        for weight_key in saved_state_dict:
            if "fc" in weight_key:
                logger.warning("skip the very last layer {} initialization..".format(weight_key))
                continue
            _query_key =  weight_key
            if _query_key in new_params.keys():
                new_params[_query_key] = saved_state_dict[
                    weight_key
                ]
                logger.info(
                    "loading weight from pretrained dictionary: {}".format(
                        weight_key
                    )
                )
            else:
                logger.warning(
                    "{} doesn't exist in your model, skip pretrained-weight initialization".format(_query_key))
...
class Net_Moco(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            return vis_and_semantic,video_cls_query, mse, word_logits
        else:
            return vis_and_semantic
