...
class Bottleneck3D(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.conv1 = nn.Conv3d(inplanes, width, kernel_size=(3, 1, 1),
                               padding=(1, 0, 0), bias=False)
        #self.bn1 = nn.BatchNorm3d(planes)
        self.bn1 = nn.BatchNorm3d(width, eps=0.001, momentum=0.01)
        ...
        self.bn2 = nn.BatchNorm3d(width, eps=0.001, momentum=0.01)
...
class ResNet3D(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.global_pooling:
            logits = logits.mean(2).mean(2).mean(2)#pooling w and h and t
...
class wideresnet5023d(Base):
    @classmethod
    def get(cls, args):
        model = ResNet3D(Bottleneck3D, [3, 4, 6, 3],num_classes=args.nclass)  # 50
        if args.pretrained:
            print("loading pretrained weight.!")
            from .resnet import wide_resnet50_2
            model2d = wide_resnet50_2(pretrained=True)
