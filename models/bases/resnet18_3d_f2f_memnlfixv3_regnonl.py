...
class ResNet3D(nn.Module):
    ...
    def forward(self, x, mv=0.9):
        ...
        for b in range(batch_size):
            tmp = -torch.norm(normalized_cls_embed[b] - self.cls_mem, p=2,dim=1)
            reg_logits[b] = tmp
        ...
        nled_logits = self.fc(self.dropout(cls_embed))#self.nled_fc(self.cls_nl(x_support=cls_embed, query=self.cls_mem))

        #normalized_rank_embed = F.normalize(rank_embed, p=2, dim=1)#[B,C,T], normalize on C dimension
        nled_embed = rank_embed#nled_embed = wrap_rankembed(_rank_embed=rank_embed)
...
class ResNet183DF2FMemNlFixv3RegNoNL(Base):
    @classmethod
    def get(cls, args):
