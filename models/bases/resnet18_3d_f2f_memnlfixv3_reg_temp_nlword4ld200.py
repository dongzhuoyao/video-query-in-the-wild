...
class SemanticAdaptor(nn.Module):
    ...
    def __init__(self,semantic_dim=1024):
        ...
        self.fc = nn.Linear(512, 640)
        self.fc2 = nn.Linear(640, 768)
        self.fc3 = nn.Linear(768, 896)
        self.fc4 = nn.Linear(896, semantic_dim)
        self.relu = nn.ReLU(inplace=True)
...
class ResNet3D(nn.Module):
    def __init__(self, block, layers, num_classes=400):
        ...
        self.rank_nl = NONLocalBlock1D(in_channels=512 * block.expansion, inter_channels=512 * block.expansion, sub_sample=False,
                                      bn_layer=True)
        self.semantic_nl = NONLocalBlock1D(in_channels=200, inter_channels=200,
                                      sub_sample=False, bn_layer=True)
        self.nled_fc = nn.Linear(512 * block.expansion, num_classes)
        self.word_adaptor = SemanticAdaptor(semantic_dim=200)
    ...
    def forward(self, x, mv=0.9):
        ...
        nled_logits = self.nled_fc(self.cls_nl(x_support=cls_embed, query=self.cls_mem))#cls_embed:[48,512];self.cls_mem:[200,512]
        ...
        if self.training:
            ...
            word_embed_pred = self.word_adaptor(cls_embed)#[48, 1024]
            word_embed_pred = self.semantic_nl(x_support=word_embed_pred, query=semantic_mem)#semantic_mem:[200,1024]
            for b in range(batch_size):
                # semantic_mem already normalized
                word_logits[b] = -torch.norm(semantic_mem
                                   - F.normalize(word_embed_pred[b], p=2, dim=-1)
                                   , p=2,dim=1)/temperature #[B,1] /

            return logits, rank_embed, nled_logits, nled_embed, reg_logits, word_logits
        else:
...
class ResNet183DF2FMemNlFixv3RegTempNLWord4LD200(Base):
    @classmethod
    def get(cls, args):
