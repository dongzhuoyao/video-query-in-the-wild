...
class SemanticAdaptor(nn.Module):
    ...
    def __init__(self):
        super(SemanticAdaptor, self).__init__()
...
class ResNet3D(nn.Module):
    def __init__(self, block, layers, num_classes=400):
        ...
        self.word_adaptor = SemanticAdaptor()
    ...
    def forward(self, x, mv=0.9):
        ...
        x, y, temperature, semantic_mem = x
        ...
        _c_num = rank_embed.size(1)
        _t_num = rank_embed.size(2)
        ...
        #normalized_rank_embed = F.normalize(rank_embed, p=2, dim=1)#[B,C,T], normalize on C dimension
        nled_embed = wrap_rankembed(_rank_embed=rank_embed)


        #self.word_adaptor
        word_logits = torch.ones([batch_size*_t_num, self.num_classes]).cuda()
        word_embed = rank_embed.permute(0,2,1).reshape(-1,_c_num)
        word_embed_pred = self.word_adaptor(word_embed)#[BT,C]
        word_embed_pred = F.normalize(word_embed_pred, p=2, dim=-1)

        if self.training:
            for b in range(word_embed_pred.size(0)):
                # semantic_mem already normalized
                word_logits[b] = - torch.norm(semantic_mem
                                   - word_embed_pred[b]
                                   , p=2,dim=1)/temperature

            word_logits = word_logits.reshape(batch_size, _t_num, self.num_classes).permute(0, 2, 1)  # [B,C,T]
            reduced_word_logits = word_logits.mean(dim=2)


        word_embed_pred = word_embed_pred.reshape(batch_size, _t_num, 1024).permute(0, 2, 1)
        nled_embed = torch.cat((F.normalize(nled_embed, p=2, dim=1),
                                word_embed_pred)
                                ,1)#torch.Size([36, 1536, 32]),make sure all of them are normalized

        nled_embed = None  # disable this variable, avoid shit
        if self.training:
            return logits, rank_embed, nled_logits, nled_embed, reg_logits, reduced_word_logits
        else:
            return logits, rank_embed, nled_logits, nled_embed, reg_logits
...
class ResNet183DF2FMemNlFixv3RegTempWordBTC(Base):
    @classmethod
    def get(cls, args):
