...
class ResNet3D(nn.Module):
    ...
    def forward(self, x, mv=0.9):
        ...
        nled_logits = self.nled_fc(self.cls_nl(x_support=embed, query=self.cls_mem))

        rank_embed = F.normalize(rank_embed, p=2, dim=1)#[B,C,T], normalize on C dimension
...
class ResNet183DF2FMemNlFixv2Reg(Base):
    @classmethod
    def get(cls, args):
