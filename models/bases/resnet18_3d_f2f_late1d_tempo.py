...
class ResNet3D(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=400):
        ...
        self.reduce_dim = nn.Conv3d(3, 64, kernel_size=(1, 1, 1), stride=(1, 2, 2),
                               padding=(0, 3, 3), bias=False)
        self.temporal_conv1 = nn.Conv1d(512 * block.expansion, 512 * block.expansion, kernel_size=3, stride=1, padding=1, bias=False)
        self.temporal_relu1 = nn.ReLU(inplace=True)
        self.temporal_conv2 = nn.Conv1d(512 * block.expansion, 512 * block.expansion, kernel_size=3, stride=1,dilation=2, padding=1, bias=False)
        self.temporal_relu2 = nn.ReLU(inplace=True)
        self.temporal_conv3 = nn.Conv1d(512 * block.expansion, 512 * block.expansion, kernel_size=3, stride=1,dilation=4, padding=1, bias=False)
...
####
        tempo_ratio = [1, 2, 4, 8]  # 32
        B = rank_embed.size(0)
        full_embed = rank_embed#B,C,T
        tmp_embed = full_embed.mean(dim=-1)#B,C
        for _b in range(B):
            _random_ratio = random.sample(tempo_ratio, k=1)[0]
            tmp_embed[_b, :] = full_embed[_b, :, ::_random_ratio].mean(dim=-1)

        logits = self.fc(self.dropout(tmp_embed))  # torch.Size([8, 200, 15, 1, 1])

        if self.training:
            return logits, tmp_embed
        else:
            return logits, full_embed
####
...
class ResNet183DF2FLate1DTempo(Base):
    @classmethod
    def get(cls, args):
