    ...
    def __init__(self, block, layers, num_classes=400):
        ...
        class TemporalModule(nn.Module):
            ...
            def forward(self, rank_embed):
                ...
                ######
                activated_len = list(range(4,25))  # 4 to 24
                B = _init_feat.size(0)
                full_embed = _init_feat  # B,C,T
                tmp_embed = full_embed.mean(dim=-1)  # B,C
                for _b in range(B):
                    random_len = random.sample(activated_len, k=1)[0]
                    random_start = random.randint(0, 32-random_len)## Integer from 1 to 10, endpoints included
                    tmp_embed[_b, :] = full_embed[_b, :, random_start:random_start+random_len].mean(dim=-1)
...
class ResNet183DF2FLate1DDropMT(Base):
    @classmethod
    def get(cls, args):
