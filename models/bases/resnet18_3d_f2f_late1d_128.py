...
class ResNet3D(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=400):
        ...
        reduce_dim_num = 128
        self.fc = nn.Linear(reduce_dim_num, num_classes)

        self.reduce_dim = nn.Conv1d(512 * block.expansion, reduce_dim_num, kernel_size=1, stride=1, padding=1, bias=False)
        self.reduce_dim_relu =  nn.ReLU(inplace=True)
        self.temporal_conv1 = nn.Conv1d(reduce_dim_num, reduce_dim_num, kernel_size=3, stride=1, padding=1, bias=False)
        self.temporal_relu1 = nn.ReLU(inplace=True)
        self.temporal_conv2 = nn.Conv1d(reduce_dim_num, reduce_dim_num, kernel_size=3, stride=1,dilation=2, padding=1, bias=False)
        self.temporal_relu2 = nn.ReLU(inplace=True)
        self.temporal_conv3 = nn.Conv1d(reduce_dim_num, reduce_dim_num, kernel_size=3, stride=1,dilation=4, padding=1, bias=False)
    ...
    def forward(self, x):
        ...
        rank_embed = self.reduce_dim_relu(self.reduce_dim(rank_embed))
...
class ResNet183DF2FLate1D128(Base):
    @classmethod
    def get(cls, args):
