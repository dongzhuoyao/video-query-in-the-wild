...
class ResNet3D(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=400):
        ...
        self.rank_embed = nn.Conv3d(512 * block.expansion, 128, kernel_size=(1, 1, 1), stride=(1, 1, 1),
    ...
    def forward(self, x):
        ...
        embed = self.rank_embed(x)
        embed = embed.view(embed.size(0), embed.size(1), -1).mean(dim=2)

        x =  x.view(x.size(0), x.size(1),  -1).mean(dim=2)#B,C
...
class ResNet183DF2FEmbed128(Base):
    @classmethod
    def get(cls, args):
