...
class ResNet3D(nn.Module):
    def __init__(self, block, layers, num_classes=400):
        ...
        self.cls_mem = nn.Parameter(torch.zeros(num_classes, 512 * block.expansion),requires_grad=True)
    ...
    def forward(self, x, mv=0.9):
        ...
        for b in range(batch_size):
            tmp = (-torch.norm(embed[b] - self.cls_mem, p=2,dim=1))
            reg_logits[b] = tmp

        if self.training:
            #with torch.no_grad():
                cloned = self.cls_mem.clone()
                for ii, _y in enumerate(y):
                    old = cloned[_y]
                    tmp = mv*old + (1-mv)*embed[ii]
                    cloned[_y] = F.normalize(tmp, p=2, dim=0)#https://discuss.pytorch.org/t/leaf-variable-was-used-in-an-inplace-operation/308/4
                self.cls_mem = cloned
        nled = self.cls_nl(x_support=embed, query=self.cls_mem)

        logits = self.fc(self.dropout(embed))  # torch.Size([8, 200, 15, 1, 1])
        nled_logits = self.nled_fc(nled)
        c_num = rank_embed.size(1)
        t_num = rank_embed.size(2)
        forinput = rank_embed.permute(0,2,1).reshape(-1, c_num)#->[B,T,C]
        nled_embed = self.rank_nl(x_support=forinput, query=self.cls_mem)
        nled_embed = nled_embed.reshape(-1,t_num,c_num)
        nled_embed = nled_embed.permute(0,2,1)#[B,C,T]
...
class ResNet183DF2FMemRegFlex(Base):
    @classmethod
    def get(cls, args):
