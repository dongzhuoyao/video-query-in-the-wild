...
class ResNet3D(nn.Module):
    ...
    def forward(self, x):
        ...
        metric_feat = x.mean(2).mean(2).mean(2)

        metric_feat = self.dropout(metric_feat)
        logits = self.fc(metric_feat)#torch.Size([8, 200, 15, 1, 1])
    ...
    def load_2d(self, model2d):
        ...
        for k, v in sd2d.items():
            ...
            if 'conv' in k or 'downsample.0' in k:
                ...
                print("loading pretrained weight: {}".format(k))
...
class ResNet1013DF2F(Base):
    @classmethod
    def get(cls, args):
        model = ResNet3D(Bottleneck3D, [3, 4, 23, 3],num_classes=args.nclass)  # 50
        if args.pretrained:
