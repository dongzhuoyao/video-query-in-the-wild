...
class ResNet3D(nn.Module):
    ...
    def forward(self, x, mv=0.9):
        ...
        rank_embed = rank_embed.permute(0,2,1).reshape(-1, c_num)#->[B,T,C]
        r_nled = self.rank_nl(x_support=rank_embed, query=self.cls_mem)
        r_nled = r_nled.reshape(-1,t_num,c_num)
        r_nled = r_nled.permute(0,2,1)
        return logits, rank_embed, nled_logits, r_nled
