...
FPS = 3
EXT = ".webm"  #'.mkv,mp4'
VIDEO_DIR = os.path.join("/home", getpass.getuser(), "dataset/v1-3/train_val/")
video_list = os.listdir(VIDEO_DIR)
META_FILE = "./activity_net.v1-3.min.json"
meta_data = json.load(open(META_FILE))
FRAME_DIR = os.path.join(
    "/home", getpass.getuser(), "dataset/v1-3/webm_train_val_frames_" + str(FPS)
)
mkdir(FRAME_DIR)
...
def ffmpeg_extract(filename, outpath):
    status = False
    outfile = os.path.join(outpath, "image_%5d.jpg")  # rescale to 171x128
    command = "ffmpeg -loglevel panic -i {} -vf scale=171:128 -q:v 1 -r {} {}".format(
        filename, FPS, outfile
    )
    # hardware accelerate
...
if __name__ == "__main__":
    ...
    print(
        "Running {} jobs, {}s per videos".format(
            n_jobs, (end - start) / len(file_list)
        )
    )
