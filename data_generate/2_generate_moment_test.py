...
print(
    "average activity duration in gallery= {} seconds".format(
        sum(segment_duration_list_4statistics) / len(segment_duration_list_4statistics)))
    ...
    for g in gallery:
        for segment in g['annotations']:
            ...
            if segment['label'] in arv_val_label:
                continue  # only consider testing data in ActivityNet
            if clip_end_idx - clip_start_idx < defined_clips_per_moment:  # grip search, most cases fall in this condition
                iou = max(
                    cal_iou(clip_start_idx, clip_end_idx, int(clip_start_idx), int(clip_end_idx)),
                    cal_iou(clip_start_idx, clip_end_idx, int(clip_start_idx) + 1, int(clip_end_idx)),
                    cal_iou(clip_start_idx, clip_end_idx, int(clip_start_idx), int(clip_end_idx) + 1),
                    cal_iou(clip_start_idx, clip_end_idx, int(clip_start_idx) + 1, int(clip_end_idx) + 1),
                )
            else:  # > or =
                iou = defined_clips_per_moment * 1.0 / (clip_end_idx - clip_start_idx)
            assert iou <= 1
            ious.append(iou)
    return ious
...
for c in clip_sec:
    for m in max_clip_per_moment:
        ious = find_closest_moment(defined_clip_sec=c, defined_clips_per_moment=m, gallery=gallery_list)
        npos = len(ious)
        ious = np.array(ious)
...
print(np.histogram(np.array(gallery_duration_list),
